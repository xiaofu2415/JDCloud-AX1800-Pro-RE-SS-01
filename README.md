...
| iStoreOS Dashboard | `0.1.0-beta.9` | beta 实验版 | QuickStart 首页；其他 LuCI 页面使用 Argon | Prerelease，不得设为 Latest |
...
两个变体都预装 PassWall2、MosDNS、AdGuard Home、nlbwmon、Tailscale、SQM、Argon、Bootstrap 和简体中文。PassWall2 的全局主开关、MosDNS、AdGuard Home、Tailscale 与 SQM 首次启动保持关闭；PassWall2 的 init 服务保持启用，因此用户在 LuCI 打开主开关并保存后可直接启动，不会再遇到“主开关已开但启动项仍禁用”。nlbwmon 默认启用并保留 3 期数据库。Docker/Dockerman 不纳入下一版基线，以减少固件体积、依赖和误启用风险。PassWall2 的首次启动策略会把内置 `DirectFront`、`DirectGame` 分流规则固定到“直连”；默认不会接管流量。
