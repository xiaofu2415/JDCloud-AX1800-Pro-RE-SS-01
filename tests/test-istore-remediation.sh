...
require_text "$repo_root/versions/istore.version" '0.1.0-rc.1'
...
formatted_channel="$temporary/channel_analysis-formatted.js"
cp "$repo_root/tests/fixtures/channel_analysis-formatted.js" "$formatted_channel"
"$channel_hardener" "$formatted_channel"
grep -Eq 'offsetWidth[[:space:]]*<[[:space:]]*1' "$formatted_channel"
grep -Eq 'initialized[[:space:]]*=[[:space:]]*true' "$formatted_channel"
grep -Eq 'freq_tbl[[:space:]]*:[[:space:]]*bands\[band\]\.channels' "$formatted_channel"
grep -Fq 'cbi-tab-active' "$formatted_channel"
cp "$formatted_channel" "$temporary/formatted-channel-after-once.js"
"$channel_hardener" "$formatted_channel"
diff -u "$temporary/formatted-channel-after-once.js" "$formatted_channel"
...
echo "iStore RC remediation contracts: ok"
