...
| iStore | `re-ss-01-istore-v0.1.0-rc.1` | `京东云 AX1800 PRO（RE-SS-01）· iStoreOS Dashboard v0.1.0-rc.1` |
...
rc.1 当前构建使用以下 iStore 文件名：

```text
jdcloud-re-ss-01-libwrt-istore-v0.1.0-rc.1-squashfs-factory.bin
jdcloud-re-ss-01-libwrt-istore-v0.1.0-rc.1-squashfs-sysupgrade.bin
```
...
- iStoreOS Dashboard RC 仍是 Prerelease，且必须显式设置为非 Latest。当前候选版本为 `0.1.0-rc.1`。
- iStore 云编译成功不会改变其候选状态。完成 RE-SS-01 真机物理验收并进入后续稳定发布决策前，Argon 仍是已验证的稳定推荐。
