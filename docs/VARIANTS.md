...
在 RE-SS-01 上只把 QuickStart 当作状态 Dashboard 使用。beta.9 延续关闭自动网络改写和 WAN 事件钩子的策略，并为 Qualcomm CPU thermal-zone 加入首页温度兼容适配；整盘容量口径仍以标准 LuCI 挂载点页面为准。不要使用 QuickStart 网络向导，所有 LAN、WAN、DHCP 与无线设置都通过标准 LuCI 完成。
