...
- iStore：`versions/istore.version`，当前 `0.1.0-beta.5`
...
5. 对 PassWall2 的运行目录/临时 Xray 链接、Samba4 rpcd ACL 和 iStore QuickStart 状态模板执行内容校验 hardener；若上游实现发生未知变化或出现危险 ACL 则停止构建。
...
即使 beta 编译成功，也必须完成真机硬件验证后，才允许合并到 `main`。物理验收至少应覆盖启动、标准 LuCI、Argon、Bootstrap、QuickStart、iStore 列表、Samba 页面、LAN 侧终端、WAN 隔离、默认服务、重启与断电重启。PassWall2 手动启动时应确认 Xray 运行目录存在；旧日志中的权限错误不能替代当前启动结果。
